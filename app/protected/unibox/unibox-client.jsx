...
function formatPhoneNumber(phone) {
  ...
  // Fallback: return original if not 11 digits
  return phone;
}
...
export default function UniboxClient({ userInboxes }) {
  ...
  // Load conversation for the selected message.
  async function loadConversation(inbox) {
    ...
    setLoadingConv(true);
    try {
      // Build the itemId by concatenating the literal "t.%252B" with the stored phone_number.
      const itemId = "t.%252B" + inbox.phone_number;
      const base_url = inbox.backend?.base_url;
      if (!base_url) {
        throw new Error("No backend URL found for this message.");
      }
      const res = await fetch(
        `${base_url}/conversation?itemId=${itemId}&account_index=${inbox.account_index}&user_id=${inbox.user_id}`
      );
  ...
  async function handleSendNew() {
    ...
    try {
      ...
      const base_url = selected.backend?.base_url;
      if (!base_url) throw new Error("No backend URL found for this message.");
    ...
    <div className="h-screen overflow-hidden flex">
      ...
      <div className="w-80 flex-shrink-0 border-r p-4">
        ...
        <div
          ...
          style={{ scrollbarWidth: "thin", scrollbarColor: "#666 #2f2f2f" }}
        >
          {sortedInboxes.map((inbox, idx) => {
            const isActive = selectedInbox && selectedInbox.id === inbox.id;
            return (
              <div
                key={idx}
              ...
              >
                ...
                {inbox.last_message_timestamp && (
                  <Text className="text-xs text-zinc-500 mt-1 mb-0">
                    {new Date(inbox.last_message_timestamp).toLocaleString()}
                  </Text>
                )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {showNewForm ? (
            ...
            <>
              <Heading level={3}>
                Conversation with {formatPhoneNumber(selectedInbox.phone_number)}
              </Heading>
              {loadingConv ? (
